...
=========================\n");
 do{
  ...
  case _OUTPUT_FILE_NAME: {
	...
	config->output_file_name = malloc(len);
	snprintf(config->output_file_name, len, "%s", s);
  ...
  case _SAVE_NEURAL_NETWORK: {
	ret = fscanf(fp, "%179s", s);
	size_t len = strlen(s) + 1;
	config->save_network_file_name = malloc(len);
	snprintf(config->save_network_file_name, len, "%s", s);
  ...
  case _LOAD_NEURAL_NETWORK: {
	ret = fscanf(fp, "%179s", s);
	size_t len = strlen(s) + 1;
	config->load_network_file_name = malloc(len);
	snprintf(config->load_network_file_name, len, "%s", s);
	config->load_neural_network = ON;
