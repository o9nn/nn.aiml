...
namespace Microsoft.Azure.Commands.Management.CognitiveServices
{
    ...
    /// </summary>
    [Cmdlet("Get", ResourceManager.Common.AzureRMConstants.AzureRMPrefix + "CognitiveServicesAccountRaiPolicy", DefaultParameterSetName = DefaultParameterSet), OutputType(typeof(RaiPolicy))]
    public class GetAzureCognitiveServicesAccountRaiPolicyCommand : CognitiveServicesAccountBaseCmdlet
    {
        ...
        public override void ExecuteCmdlet()
        {
            ...
            RunCmdLet(() =>
            {
                ...
                {
                    var getResponse = ListAll(CognitiveServicesClient.RaiPolicies.List, ResourceGroupName, AccountName, CognitiveServicesClient.RaiPolicies.ListNext);
                    WriteObject(getResponse);
                }
                else
                {
                    var getResponse = CognitiveServicesClient.RaiPolicies.Get(ResourceGroupName, AccountName, Name);
                    WriteObject(getResponse);
                }
            });
        }
