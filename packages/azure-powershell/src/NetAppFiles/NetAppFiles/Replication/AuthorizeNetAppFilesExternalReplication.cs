...
namespace Microsoft.Azure.Commands.NetAppFiles.Replication
{
    [Cmdlet(
        "Start",
        ResourceManager.Common.AzureRMConstants.AzureRMPrefix + "NetAppFilesAuthorizeExternalReplication",
        SupportsShouldProcess = true,
        DefaultParameterSetName = FieldsParameterSet), OutputType(typeof(bool))]
    [Alias("Start-AnfAuthorizeExternalReplication")]
    public class AuthorizeAzureRmNetAppFilesExternalReplication : AzureNetAppFilesCmdletBase
    {
        [Parameter(
            Mandatory = true,
        ...
        public override void ExecuteCmdlet()
        {
            ...
            if (ShouldProcess(Name, string.Format(PowerShell.Cmdlets.NetAppFiles.Properties.Resources.ApproveVolumeReplicationMessage, Name)))
            {
                try
                {
                    var svmCommandResponse = AzureNetAppFilesManagementClient.Volumes.AuthorizeExternalReplication(ResourceGroupName, AccountName, PoolName, Name);
                    WriteObject(svmCommandResponse.ConvertToPs());
                    success = true;
                }
