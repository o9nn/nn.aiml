...
{
    [Cmdlet("Remove", ResourceManager.Common.AzureRMConstants.AzureRMPrefix + "CosmosDBTableRoleAssignment", DefaultParameterSetName = ResourceIdParameterSet, SupportsShouldProcess = true), OutputType(typeof(void))]
    public class RemoveAzCosmosDBTableRoleAssignment : AzureCosmosDBCmdletBase
    {
        ...
        public override void ExecuteCmdlet()
        {
            ...
            if (ShouldProcess(Id, "Deleting CosmosDB Table Role Assignment"))
            {
                CosmosDBManagementClient.TableResources.DeleteTableRoleAssignmentWithHttpMessagesAsync(ResourceGroupName, AccountName, TableRoleHelper.ParseToRoleAssignmentId(Id)).GetAwaiter().GetResult();
