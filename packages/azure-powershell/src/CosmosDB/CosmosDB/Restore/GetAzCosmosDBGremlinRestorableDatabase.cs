...
{
    [Cmdlet(VerbsCommon.Get, ResourceManager.Common.AzureRMConstants.AzureRMPrefix + "CosmosDBGremlinRestorableDatabase", DefaultParameterSetName = NameParameterSet), OutputType(typeof(PSRestorableGremlinDatabaseGetResult))]
    public class GetAzCosmosDBGremlinRestorableDatabase : AzureCosmosDBCmdletBase
    {
        ...
        public override void ExecuteCmdlet()
        {
            ...
            IEnumerable restorableGremlinDatabases = CosmosDBManagementClient.RestorableGremlinDatabases.ListWithHttpMessagesAsync(Location, DatabaseAccountInstanceId).GetAwaiter().GetResult().Body;
            foreach (RestorableGremlinDatabaseGetResult restorableGremlinDatabase in restorableGremlinDatabases)
            {
                WriteObject(new PSRestorableGremlinDatabaseGetResult(restorableGremlinDatabase));
            }
        }
    }
}
