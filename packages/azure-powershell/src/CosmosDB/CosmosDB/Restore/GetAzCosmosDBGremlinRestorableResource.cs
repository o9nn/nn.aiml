...
{
    [Cmdlet(VerbsCommon.Get, ResourceManager.Common.AzureRMConstants.AzureRMPrefix + "CosmosDBGremlinRestorableResource", DefaultParameterSetName = NameParameterSet), OutputType(typeof(PSGremlinDatabaseToRestore))]
    public class GetAzCosmosDBGremlinRestorableResource : AzureCosmosDBCmdletBase
    {
        ...
        public override void ExecuteCmdlet()
        {
            ...
            IEnumerable restorableGremlinResources = CosmosDBManagementClient.RestorableGremlinResources.ListWithHttpMessagesAsync(Location, DatabaseAccountInstanceId, RestoreLocation, dateTimeInUtc.ToString()).GetAwaiter().GetResult().Body;
            foreach (RestorableGremlinResourcesGetResult restorableGremlinResource in restorableGremlinResources)
            {
                WriteObject(new PSGremlinDatabaseToRestore(new GremlinDatabaseRestoreResource(restorableGremlinResource.DatabaseName, restorableGremlinResource.GraphNames)));
            }
        }
    }
}
