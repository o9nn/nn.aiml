...
{
    [Cmdlet("Lock", ResourceManager.Common.AzureRMConstants.AzureRMPrefix + "SqlDatabaseLongTermRetentionBackupImmutability", DefaultParameterSetName = LockBackupDefaultSet, SupportsShouldProcess = true), OutputType(typeof(AzureSqlDatabaseLongTermRetentionBackupModel))]
    public class LockAzureRmSqlDatabaseLongTermRetentionBackupImmutability : AzureSqlDatabaseLongTermRetentionBackupCmdletBase<AzureSqlDatabaseLongTermRetentionBackupModel>
    {
        /// <summary>
        /// Parameter set name for the default lock.
        /// </summary>
        private const string LockBackupDefaultSet = "LockBackupDefault";

        /// <summary>
        /// Parameter set name for lock with an input object.
        /// </summary>
        private const string lockBackupByInputObjectSet = "LockBackupByInputObject";

        /// <summary>
        /// Parameter set name for lock with a resource ID.
        /// </summary>
        private const string LockBackupByResourceIdSet = "LockBackupByResourceId";

        /// <summary>
        /// Gets or sets the name of the location the backup is in.
        /// </summary>
        [Parameter(Mandatory = true,
            ParameterSetName = LockBackupDefaultSet,
            Position = 0,
        ...
        /// <summary>
        /// Gets or sets the LTR Backup object to lock.
        /// </summary>
        [Parameter(ParameterSetName = lockBackupByInputObjectSet,
            Mandatory = true,
            Position = 0,
        ...
        /// <summary>
        /// Gets or sets the resource ID of the LTR Backup to lock.
        /// </summary>
        [Parameter(ParameterSetName = LockBackupByResourceIdSet,
            Mandatory = true,
            Position = 0,
        ...
        public override void ExecuteCmdlet()
        {
            ...
            if (ShouldProcess(this.BackupName))
            {
                if (Force.IsPresent || ShouldContinue(
                    string.Format(CultureInfo.InvariantCulture, Properties.Resources.LockAzureSqlDatabaseLongTermRetentionBackupWarning, this.BackupName, this.DatabaseName, this.ServerName, this.Location),
                    string.Format(CultureInfo.InvariantCulture, Properties.Resources.LockAzureSqlDatabaseLongTermRetentionBackupDescription, this.BackupName, this.DatabaseName, this.ServerName, this.Location)))
                {
                    base.ExecuteCmdlet();
                }
            }
